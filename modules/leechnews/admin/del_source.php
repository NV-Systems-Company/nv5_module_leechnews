...
if( !empty( $del_array ) )
{
	$sql = 'SELECT id, title FROM ' . NV_PREFIXLANG . '_' . $module_data . '_source WHERE id IN (' . implode( ',', $del_array ) . ')';
	$result = $db->query( $sql );
	...
	while( list( $id, $title ) = $result->fetch( 3 ) )
	{
		...
		{
			$contents = nv_del_content_source( $id );
			$artitle[] = $title;
			$del_array[] = $id;
		}
		else
	...
	if( $count )
	{
		nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['del_source'], implode( ', ', $artitle ), $admin_info['userid'] );
	}
